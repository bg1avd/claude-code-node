...
describe('isLocalHostname', () => {
  const localCases = ['localhost', '127.0.0.1', '192.168.1.191', '10.0.0.5', '172.20.1.1',
...
describe('isLocalLlmServer', () => {
  ...
  test('Ollama 内网', () => assert.strictEqual(isLocalLlmServer('http://192.168.1.191:11434/v1'), true))
  ...
  test('无端口内网', () => assert.strictEqual(isLocalLlmServer('http://192.168.1.191/v1'), true))
  test('.local 域名', () => assert.strictEqual(isLocalLlmServer('http://myhost.local:11434/v1'), true))
  test('无 /v1 前缀也可', () => assert.strictEqual(isLocalLlmServer('http://192.168.1.191:11434'), true))
...
describe('buildAuthHeaders', () => {
  test('本地服务 + 有 key → 带 Bearer', () => {
    assert.deepStrictEqual(buildAuthHeaders('http://192.168.1.191:11434/v1', 'secret'),
      { 'Authorization': 'Bearer secret' })
  })
  test('本地服务 + 无 key → 空对象(不带Authorization)', () => {
    assert.deepStrictEqual(buildAuthHeaders('http://192.168.1.191:11434/v1', ''), {})
  })
