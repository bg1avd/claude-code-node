...
function add(input) {
  ...
  if (!text) throw new Error('text 必填(提醒内容或要执行的指令)')
  ...
  return {
    ok: true, id,
    desc: `${spec.kind === 'once' ? '一次性' : spec.kind === 'daily' ? `每天 ${input.timeOfDay}` : `每 ${spec.everyMs / 60000} 分钟`} — ${text}`,
...
使用方法:
  ...
  actionType: "tg"(默认,到期发 Telegram 提醒)| "prompt"(到期让 AI 执行 text)
...
- 每 30 分钟盯一次回测日志: { "text": "检查回测 log,完成则汇总发我", "kind": "every", "every": "30m", "actionType": "prompt" }`,
    {
      ...
      properties: {
        ...
        actionType: { type: 'string', enum: ['tg', 'prompt'], description: 'tg=发提醒;prompt=让 AI 执行' },
