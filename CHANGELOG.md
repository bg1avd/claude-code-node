...
- **fix(ui)**: 修复终端输出一旦遇到 ANSI 颜色（如 npm warning 黄色）就永久卡死的 bug
- 根因：工具输出携带的 ANSI 序列被截断逻辑拦腰切断（只有开启无复位），或被模型复述后原样写终端
- 修复（三层防御）：工具结果/用户输入入库前剥离 ANSI；流式与非流式输出终端直写前剥离；提示符显示前强制复位
- 附带收益：ANSI 序列不再进入模型上下文，节省 token
- 新增 8 项单元测试
...
- **fix(model)**: 修复"多模态模型发图识别 → /model 切回纯文字模型 → 之后所有消息被 API 以 image not supported 拒绝"的 bug
- `/model` 切换时自动剥离会话历史中的图片并提示移除张数
- 兜底：API 报图片相关错误时自动剥离请求体+历史图片并重试（覆盖其他触发路径，只做一次防死循环）
- 新增 8 项单元测试
...
- note: 3.0.1 / 3.0.2 在 npm 侧均卡在 staged 状态无法上线（新版 npm CLI 分阶段发布被 2FA 拦在 finalize），版本号跳过，实发 3.0.3（同 2.6.0 先例）
